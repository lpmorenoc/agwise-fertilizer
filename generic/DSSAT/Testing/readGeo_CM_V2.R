...
#print("i am here")
  if(AOI == TRUE){
    Rainfalldata <- pivot_longer(Rainfalldata,
                             cols=-c("longitude", "latitude","NAME_1","NAME_2","startingDate", "endDate", "Zone", "country"),
...
readGeo_CM <- function(country, useCaseName, Crop, AOI = FALSE, season=1, Province){
  cat(Province)
  #pathIn <- paste("~/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_", useCaseName,"/", Crop, "/raw/geo_4cropModel/", Province,'/' ,sep="")
  pathIn <- paste("~/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_", useCaseName,"/", Crop, "/result/geo_4cropModel/", Province,'/' ,sep="")
  
  if(AOI == TRUE & !is.na(Province)){
    Rainfall <- readRDS(paste(pathIn, "Rainfall_Season_", season, "_PointData_AOI.RDS", sep=""))
    # Rainfall <- Rainfall[Rainfall$Zone == Province, ]
    Rainfall <- Rainfall[Rainfall$Zone == Province, ]
          #cat("Rain done")
    SolarRadiation <- readRDS(paste(pathIn, "solarRadiation_Season_", season, "_PointData_AOI.RDS", sep=""))
    SolarRadiation <- SolarRadiation[SolarRadiation$Zone == Province, ]
    #cat("sr done")
      TemperatureMax <- readRDS(paste(pathIn, "temperatureMax_Season_", season, "_PointData_AOI.RDS", sep=""))
    TemperatureMax <- TemperatureMax[TemperatureMax$Zone == Province, ]
    #cat("tm done")
      TemperatureMin <- readRDS(paste(pathIn, "temperatureMin_Season_", season, "_PointData_AOI.RDS", sep=""))
    TemperatureMin <- TemperatureMin[TemperatureMin$Zone == Province, ]
    #cat("tmin done")
    ...
    # cat("rh done")
      ...
      #z<-which(colnames(Soil) %in% c('altitude','slope','TPI','TRI'))
      #Soil <- na.omit(Soil[,-z])
  }else{
  ...
  names(Rainfall)[names(Rainfall)=="lat"] <- "latitude"
  names(Rainfall)[names(Rainfall)=="lon"] <- "longitude"
  names(SolarRadiation)[names(SolarRadiation)=="lat"] <- "latitude"
  names(SolarRadiation)[names(SolarRadiation)=="lon"] <- "longitude"
  names(TemperatureMax)[names(TemperatureMax)=="lat"] <- "latitude"
  names(TemperatureMax)[names(TemperatureMax)=="lon"] <- "longitude"
  names(TemperatureMin)[names(TemperatureMin)=="lat"] <- "latitude"
  names(TemperatureMin)[names(TemperatureMin)=="lon"] <- "longitude"
  ...
  if(AOI == TRUE){
    #metaDataWeather <- as.data.frame(Rainfall[,c("lon", 'lat', "startingDate", "endDate", "ID", "Zone", "NAME_2")])
    # metaDataWeather <- as.data.frame(Rainfall[,c("longitude", 'latitude', "startingDate", "endDate", "Zone", "NAME_2")])
    metaDataWeather <- as.data.frame(Rainfall[,c("longitude", 'latitude', "startingDate", "endDate", "Zone", "NAME_2")])
  }else{
...
# return(list())
 ...
 #process_grid_element(i,country,path.to.extdata,path.to.temdata,Tmaxdata,Tmindata,Sraddata,Rainfalldata,RelativeHum,coords,Soil,AOI = TRUE)
