...
#' @param plantingWindow number of weeks that define the planting window  oordonsidering the Planting_month_date as earliest planting week. It is given when several planting dates are to be tested to determine optimal planting date (applies to AOI)  
...
create_filex <-function(i,path.to.temdata, filex_temp, path.to.extdata,coords, AOI, code,plantingWindow, number_years, ingenoid, Province, fertilizer =T){
  ...
  if(AOI==TRUE){
    ...
    # file_x$`INITIAL CONDITIONS`$SH2O<- (as.numeric(as.character((ex_profile$SDUL))))*0.5 #Assume field capacity as initial condition
    ...
    for (j in 1:plantingWindow){
      ...
      #file_x$`TREATMENTS                        -------------FACTOR LEVELS------------`[1+j,]$IC <- 1+j
       file_x$`TREATMENTS                        -------------FACTOR LEVELS------------`[j,]$IC <-  0
       # file_x$`TREATMENTS                        -------------FACTOR LEVELS------------`[1+j,]$IC <-  0
...
dssat.expfile <- function(country, useCaseName, Crop, AOI = FALSE,filex_temp, Planting_month_date=NULL,Harvest_month_date=NULL, ID="TLID",season =NULL, plantingWindow=1, ingenoid, Province, fertilizer=F){  #xmin,xmax,ymin,ymax,res,jobs,ex.name,path.to.extdata){
  if(AOI == TRUE){
    ...
    }else if(plantingWindow > 5 & plantingWindow <=13){
  ...
  pathIn <- paste("/home/jovyan/agwise-datasourcing/dataops/datasourcing/Data/useCase_", country, "_",useCaseName, "/", Crop, "/result/geo_4cropModel/", Province,sep="")
  ...
  if(AOI==TRUE){
    ...
    number_years <- 23 #This has been hard coded. Line 236 is the automated procedure.
    metaData <- unique(metaData)
  }else{
    number_years <- 1
    #metaData <- subset(metaData,select=-ID)
  }
  coords <- merge(metaData[,-c(4:5)], ground, by =c('longitude','latitude'))
  ...
  cropcode <- c("MZ","PT", "RI", "SB", "WH", "BN","CS")
