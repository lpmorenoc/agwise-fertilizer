...
create_filex <-function(i,path.to.temdata,filex_temp,path.to.extdata,coords, AOI, code,plantingWindow,number_years,ingenoid, Province, fertilizer =T){
  ...
  if(AOI==TRUE){
    ...
    setwd(paste(path.to.extdata,paste0(Province,'/AOI/EXTE', formatC(width = 4, (as.integer(i)), flag = "0")), sep = "/"))
...
dssat.expfile <- function(country, useCaseName, Crop, AOI = FALSE,filex_temp, Planting_month_date=NULL,Harvest_month_date=NULL, ID="TLID",season =NULL, plantingWindow=1, ingenoid, Province, fertilizer=F){  #xmin,xmax,ymin,ymax,res,jobs,ex.name,path.to.extdata){
  ...
  if(AOI == TRUE){
    path.to.extdata <- paste("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/",ingenoid, sep="")
  }else{
    path.to.extdata <- paste("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", country, "_",useCaseName, "/", Crop, "/transform/DSSAT/fieldData/",ingenoid, "/", Province, sep="")
  }
