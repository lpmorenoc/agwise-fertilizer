...
####
## Modified readGeo_CM_zone for NASA POWER Data
## Updated to work with new weather data structure from NASA POWER
####

## sourcing required packages
####
packages_required <- c("doParallel", "foreach", "tidyverse", "dplyr", "lubridate", "stringr","sf","purrr","DSSAT")
...
#' Function that creates the soil and weather file for one location/folder
#' UPDATED FOR NASA POWER DATA
#'
#' @param i last digits of the folder (folder ID)
#' @param country country name
#' @param path.to.extdata working directory to save the weather and soil data in DSSAT format
#' @param path.to.temdata directory with template weather and soil data in DSSAT format
#' @param WeatherData_LONG dataframe with all weather variables in long format from NASA POWER
#' @param coords dataframe with the locations and metadata
#' @param Soil dataframe with the soil data information
#' @param AOI True if the data is required for target area, and false if it is for trial sites
#' @return soil and weather file in DSSAT format
#' @export
#'
#' @examples process_grid_element_NASA(1)

process_grid_element_NASA <- function(i, country, path.to.extdata, path.to.temdata, 
                                      WeatherData_LONG, coords, Soil, AOI) {
  ...
  # Filter weather data for this specific location
  WeatherData <- WeatherData_LONG %>%
    filter(lon == coords$lon[i] & lat == coords$lat[i])
  
  if(nrow(WeatherData) == 0) {
    message(paste("No weather data for location", i))
    return(NULL)
  }
  
  # Prepare weather data for DSSAT
  # NASA POWER provides: T2M_MAX, T2M_MIN, PRECTOTCORR, ALLSKY_SFC_SW_DWN
  tst <- WeatherData %>%
    mutate(
      DATE = as.POSIXct(paste(YEAR, MM, DD, sep = "-"), format = "%Y-%m-%d", tz = "UTC"),
      TMAX = T2M_MAX,
      TMIN = T2M_MIN,
      RAIN = PRECTOTCORR,
      SRAD = ALLSKY_SFC_SW_DWN
    ) %>%
    select(DATE, TMAX, TMIN, SRAD, RAIN) %>%
    mutate(across(c(TMAX, TMIN, SRAD, RAIN), as.numeric)) %>%
    na.omit()
  
  if(nrow(tst) == 0) {
    message(paste("No valid weather data after processing for location", i))
    return(NULL)
  }
  ...
  amp <- tst %>%
    ...
    group_by(month) %>%
    summarise(monthly_avg = mean((TMAX + TMIN) / 2, na.rm = TRUE)) %>%
    summarise(AMP = (max(monthly_avg, na.rm = TRUE) - min(monthly_avg, na.rm = TRUE)) / 2)
  
  # Location (NAME_2)
  INS <- toupper(substr(unique(WeatherData$NAME_2), start = 1, stop = 4))
  
  # Generate new general information table
  general_new <- tibble(
    INSI = INS,
    LAT = as.numeric(coords$lat[i]),
    LONG = as.numeric(coords$lon[i]),
    TAV = tav$TAV,
    AMP = amp$AMP,
    REFHT = 2,
    WNDHT = 2
  ...
  soil_row <- Soil %>% filter(longitude == coords$lon[i] & latitude == coords$lat[i])
  
  if(nrow(soil_row) == 0) {
    message(paste("No soil data for location", i))
    return(NULL)
  }
  
  LL15 <- as.numeric(soil_row[, c("PWP_0-5cm", "PWP_5-15cm", "PWP_15-30cm", "PWP_30-60cm", "PWP_60-100cm", "PWP_100-200cm")])
  DUL  <- as.numeric(soil_row[, c("FC_0-5cm", "FC_5-15cm", "FC_15-30cm", "FC_30-60cm", "FC_60-100cm", "FC_100-200cm")])
  SAT  <- as.numeric(soil_row[, c("SWS_0-5cm", "SWS_5-15cm", "SWS_15-30cm", "SWS_30-60cm", "SWS_60-100cm", "SWS_100-200cm")])
  SKS  <- as.numeric(soil_row[, c("KS_0-5cm", "KS_5-15cm", "KS_15-30cm", "KS_30-60cm", "KS_60-100cm", "KS_100-200cm")]) / 10
  SSS  <- round(as.numeric(SKS), digits = 1)
  BDM  <- as.numeric(soil_row[, c("bdod_0-5cm", "bdod_5-15cm", "bdod_15-30cm", "bdod_30-60cm", "bdod_60-100cm", "bdod_100-200cm")])
  LOC  <- as.numeric(soil_row[, c("soc_0-5cm", "soc_5-15cm", "soc_15-30cm", "soc_30-60cm", "soc_60-100cm", "soc_100-200cm")]) / 10
  LCL  <- as.numeric(soil_row[, c("clay_0-5cm", "clay_5-15cm", "clay_15-30cm", "clay_30-60cm", "clay_60-100cm", "clay_100-200cm")])
  LSI  <- as.numeric(soil_row[, c("silt_0-5cm", "silt_5-15cm", "silt_15-30cm", "silt_30-60cm", "silt_60-100cm", "silt_100-200cm")])
  LNI  <- as.numeric(soil_row[, c("nitrogen_0-5cm", "nitrogen_5-15cm", "nitrogen_15-30cm", "nitrogen_30-60cm", "nitrogen_60-100cm", "nitrogen_100-200cm")]) / 10
  LHW  <- as.numeric(soil_row[, c("phh2o_0-5cm", "phh2o_5-15cm", "phh2o_15-30cm", "phh2o_30-60cm", "phh2o_60-100cm", "phh2o_100-200cm")])
  LDR  <- as.numeric(soil_row[, "LDR"])
  CEC  <- as.numeric(soil_row[, c("cec_0-5cm", "cec_5-15cm", "cec_15-30cm", "cec_30-60cm", "cec_60-100cm", "cec_100-200cm")])
  ...
  SLU <- slu1(soil_row[, "clay_0-5cm"], soil_row[, "sand_0-5cm"])
  ...
  soilid <- ex_profile %>%
    mutate(
      PEDON = paste0('TRAN', formatC(width = 5, (as.integer(i)), flag = "0")),
      SOURCE = "ISRIC V2",
      TEXTURE = texture_soil,
      DESCRIPTION = texture,
      SITE = substr(soil_row$NAME_2, start = 1, stop = 6),
      COUNTRY = country,
      LAT = as.numeric(soil_row$latitude),
      LONG = as.numeric(soil_row$longitude),
      SALB = list(ALB),
      SLU1 = list(SLU),
      SLRO = list(LRO),
      SLDR = list(LDR),
...
#' Reading NASA POWER weather data and soil data for crop model and transforming it to DSSAT format
#' UPDATED VERSION FOR NASA POWER DATA
#'
#' @param country country name
#' @param useCaseName use case name
#' @param Crop the name of the crop to be used in creating file name to write out the result
#' @param AOI True if the data is required for target area, and false if it is for trial sites
#' @param zone County/Province name (e.g., "Kwale", "Baringo")
#' @param weather_source Path to NASA POWER weather data folder
#'
#' @return weather and soil data in DSSAT format
#' @export
#'
#' @examples readGeo_CM_NASA(country = "Kenya", useCaseName = "Mordecai", Crop = "Maize", AOI = TRUE, zone = "Kwale")

readGeo_CM_NASA <- function(country, useCaseName, Crop, AOI = FALSE, zone, 
                            weather_source = NULL) {
  ...
  # Path to NASA POWER weather data
  if(is.null(weather_source)) {
    weather_path <- paste0("~/agwise-datasourcing/dataops/datasourcing/Data/useCase_", 
                           country, "_", useCaseName, "/", Crop, 
                           "/result/NASA_POWER_Weather/", zone, "/")
  } else {
    weather_path <- paste0(weather_source, "/", zone, "/")
  }
  
  # Path to soil data (assuming same structure as before)
  pathIn <- paste0("~/agwise-datasourcing/dataops/datasourcing/Data/useCase_", 
                   country, "_", useCaseName, "/", Crop, "/result/geo_4cropModel/", zone, "/")
  
  ####
  ## 2. Load NASA POWER weather data (LONG format)
  ####
  
  weather_file_long <- paste0(weather_path, "Weather_NASA_POWER_", zone, "_2000_2024_LONG.RDS")
  
  if(!file.exists(weather_file_long)) {
    stop(paste("Weather file not found:", weather_file_long))
  }
  
  cat("Loading NASA POWER weather data...\n")
  WeatherData_LONG <- readRDS(weather_file_long)
  
  # Ensure column names are correct
  names(WeatherData_LONG)[names(WeatherData_LONG) == "lon"] <- "lon"
  names(WeatherData_LONG)[names(WeatherData_LONG) == "lat"] <- "lat"
  ...
  ####
  ## 4. Create metadata and merge
  ####
  
  metaDataWeather <- WeatherData_LONG %>%
    select(lon, lat, country, NAME_1, NAME_2) %>%
    distinct()
  
  metaData_Soil <- Soil %>%
    select(longitude, latitude, NAME_1, NAME_2) %>%
    distinct()
  
  # Merge weather and soil metadata
  metaData <- metaDataWeather %>%
    inner_join(metaData_Soil, by = c("lon" = "longitude", "lat" = "latitude", "NAME_1", "NAME_2"))
  
  cat(paste("Locations with both weather and soil data:", nrow(metaData), "\n"))
  
  # Keep only soil data that has weather data
  Soil <- Soil %>%
    inner_join(metaData, by = c("longitude" = "lon", "latitude" = "lat", "NAME_1", "NAME_2"))
  
  # Keep only weather data that has soil data
  WeatherData_LONG <- WeatherData_LONG %>%
    inner_join(metaData, by = c("lon", "lat", "NAME_1", "NAME_2"))
  ...
  path.to.extdata <- paste0("/home/jovyan/agwise-potentialyield/dataops/potentialyield/Data/useCase_", 
                            country, "_", useCaseName, "/", Crop, "/transform/DSSAT/", zone)
  ...
  cat(paste("Total locations to process:", nrow(coords), "\n"))
  
  ####
  ## 7. Process each location
  ####
  
  cat("Processing locations...\n")
  
  results <- map(seq_along(coords$lon), process_grid_element_NASA, 
                 country = country, 
                 ...
                 coords = coords, 
                 Soil = Soil, 
                 AOI = AOI) %||% print("Progress:")
  
  cat(paste("\n=== Completed processing zone:", zone, "===\n"))
  cat(paste("Output directory:", path.to.extdata, "\n"))
...
# For a single county
# readGeo_CM_NASA(country = "Kenya", useCaseName = "Mordecai", Crop = "Maize", 
#                 AOI = TRUE, zone = "Kwale")

# For multiple counties
# counties <- c("Kwale", "Baringo", "Bungoma", "Kakamega")
# for(county in counties) {
#   readGeo_CM_NASA(country = "Kenya", useCaseName = "Mordecai", Crop = "Maize", 
#                   AOI = TRUE, zone = county)
# }