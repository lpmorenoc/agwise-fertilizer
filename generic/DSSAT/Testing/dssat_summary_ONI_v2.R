...
merge_DSSAT_output <- function(country, useCaseName, Crop, Extent, Season){
  ...
  # f_all <- NULL
  # 
  # basesub <- vector("list",length(list.exte))
  # 
  # #for (i in 1:20){
  # #for (i in 1:100){
  # # Read the exte file
  ...
  # # hd <- unlist(strsplit(whh, '[ ]'))
  # # hv <- unlist(strsplit(wh, '[ ]'))
  # # names(hv) <- hd
  # # wh <- readLines(list.wh[i], n=5)[5]
  # # 
  ...
  # base.sub$Loc <- strsplit(base, "/")[[1]][3]
  # 
  # #f_all <- dplyr::bind_rows(f_all, base.sub)
  # basesub[[i]] <- base.sub
  # #}
  # #f_all <- do.call(rbind, basesub)
  # #data.table_rbindlist = as.data.frame(data.table::rbindlist(basesub))
  # 
  # 
  # #extw <- foreach(i in 1:length(list.exte)) %do% {# Read the exte file
  # base <- list.exte[i]
  # base.r <- read_output(base)
  # # Keep the usefull information
  # base.sub <- base.r[,c("XLAT","LONG","TRNO","TNAM","PDAT","ADAT","MDAT","HDAT","CWAM","HWAH","CNAM","GNAM","NDCH","TMAXA",
  ...
  # #basesub[[i]] <- base.sub
  # #}
  #system.time(f_all <- foreach::foreach(i=1:length(list.exte)) %do% {# Read the exte file
    # #base <- list.exte[i]
    # if(i %% 1000 < 1){
    ...
    #basesub[[i]] <- base.sub
  #})
f_all <- foreach::foreach(i=1:length(list.exte)) %do% {
  #f_all <- foreach::foreach(i=1:2000) %do% {
    base <- list.exte[i]
    if(i %% 1000 < 1){
      msg_txt <- paste0(i, " of ", length(list.exte))
      #msg_txt <- paste0(i, " of ", ' f10')
