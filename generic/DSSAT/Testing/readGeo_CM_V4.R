...
#print("i am here")
  if(AOI == TRUE){
    Rainfalldata <- pivot_longer(Rainfalldata,
                             cols=-c("longitude", "latitude","NAME_1","NAME_2","startingDate", "endDate", "ID", "country"),
    ...
    Sraddata <- pivot_longer(Sraddata,
                             cols=-c("longitude", "latitude","NAME_1","NAME_2","startingDate", "endDate", "NAME_1", "country"),
                                   names_to = c("Variable", "Date"),
                                   names_sep = "_",
                                   values_to = "SRAD")
...
readGeo_CM <- function(country, useCaseName, Crop, AOI = FALSE, season=1, Province){
  ...
  if(AOI == TRUE){
    #metaDataWeather <- as.data.frame(Rainfall[,c("lon", 'lat', "startingDate", "endDate", "ID", "NAME_1", "NAME_2")])
