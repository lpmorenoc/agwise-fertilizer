...
rundssat <-function(i,path.to.extdata,TRT,AOI){
  ...
  filename <- paste0(path.to.extdata, '/', 'EXTE', formatC(width = 4, as.integer((i)), flag = "0"), '/DSSBatch.v48')
  tibble(FILEX=paste0('EXTE', formatC(width = 4, as.integer((i)), flag = "0"),'.PTX'), TRTNO=TRT, RP=1, SQ=0, OP=0, CO=0) %>%
    write_dssbatch(file_name=filename)
  
  # Run DSSAT-CSM
  run_dssat(file_name=filename,suppress_output = TRUE)
